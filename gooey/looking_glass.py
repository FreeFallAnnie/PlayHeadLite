...
def save_to_db(user_input, husky_id, response, color, db_path="how_far_we_come.db"):
    ...
    timestamp = datetime.now().isoformat(timespec='seconds')
    cursor.execute("""
        INSERT INTO responses (timestamp, husky_id, user_input, response, color)
        VALUES (?, ?, ?, ?, ?)
    """, (timestamp, husky_id, user_input, response, color))
...
def ali_start_callback(user_input, husky_id, decision):
    if decision == 'DISCARD':
        return "Discarded.", "Gray"

    husky_map = {
        1: ("Explain why.", "PINK"),
        2: ("Connect a why to a how.", "PURPLE"),
        3: ("Explain how.", "BLUE"),
        4: ("Connect a how to a what.", "GREEN"),
        5: ("Explain what.", "YELLOW"),
        6: ("Connect a what to a who.", "ORANGE")
    }

    prompt, color = husky_map.get(husky_id, ("Describe this.", "BLUE"))
    full_prompt = f"{prompt} {user_input}"
    response = ask_AliN(full_prompt)
